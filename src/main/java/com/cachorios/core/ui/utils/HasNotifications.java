...
import com.cachorios.core.utils.AppConst;
...
public interface HasNotifications extends HasElement {
	...
	default void showNotification(String message, boolean persistent) {
		if (persistent) {
			...
			Div content = new Div();
			content.addClassName("notificacion");
			content.setText(message);
			Notification noti = new Notification(content);
			noti.setDuration(AppConst.NOTIFICATION_DURATION);
			noti.setPosition(Position.BOTTOM_STRETCH);
			noti.open();
